        ...
        <div className="itali-card-image" >
            {
                Food2.map((ele)=>{
                    ...
                    <button className="slide-cart-button" onClick={order}>Đặt món</button>{'  '}
                    
                    <button className="slide-cart-button" onClick={()=>AddtoCart(ele)}>+Thêm giỏ hàng</button>
                </div>
                })
            }
    <button onClick={()=>Alldish(Food2[0].titleId) } className='imsa' >Nhiều hơn</button>
        </div>
        </div>
        </div>
    )
