...
function Cart(){
    ...
    return(
        <div className="cart-bg">
            <Header />
            <div className="cart"><h1 style={{padding:'10px'}}>Giỏ hàng</h1>
            {
                cart.cartItems.length===0 ?(
                    ...
                    <div className="cart-main">
                        <div className="cart-main-head">
                            <h3 className="cart-main-head-h3">Sản phẩm</h3>
                            <h3>Giá</h3>
                            <h3>Sốlượng</h3>
                            <h3>Tất cả</h3>
                       </div>

                        {
                            cart.cartItems?.map(cartItems=>(
                                    <div key={cartItems.id} className="cart-main-body">
                                     ...
                                     <button onClick={()=>remove(cartItems)}>Xóa</button>
                                     </div>
                                     </div>

                                     <div className="cart-main-body-div2"><h5>{cartItems.rate} VND</h5></div>
                                     ...
                                     <div className="cart-main-body-div2">
                                        <div style={{color:'green',fontSize:'23px'}}>{cartItems.cartQuantity*cartItems.rate} VND </div>
                                     </div>
                                    </div>
                            ))
                        }
                        <div style={{display:'flex',justifyContent:'space-between',width:'1100px',marginLeft:'10px'}}>
                            <div>
                               <button className="clearCart-button" onClick={()=>clearcart()}> Xoá hết  </button>
                            </div>
                            <div>
                                <p>Tổng số tiền <span style={{fontSize:'12px'}}> </span>: <b><span style={{fontSize:'23px'}}> {cart.totalAmount} VND </span></b></p>
                               
                                <button className="Order-button " onClick={order}>Đặt món </button>
                            </div>
                        </div>
                    </div>
                )
